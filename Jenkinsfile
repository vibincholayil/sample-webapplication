pipeline {
    ...
    environment {
        ...
        DOCKER_CRED_ID = 'docker-credentials' // Jenkins credential ID for DockerHub
        BUILD_ID_CUSTOM = sh(script: "date +%Y%m%d.%H%M", returnStdout: true).trim()
        IMAGE_TAG = "${DOCKERHUB_USER}/sample-webapplication:${BUILD_ID_CUSTOM}"
    }
